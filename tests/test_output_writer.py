...
def test_effective_config_metadata_summary_and_subdirectories_are_saved(tmp_path: Path) -> None:
    ...
    assert manager.extracted_frames_directory.exists()
    assert manager.yolo_detected_frames_directory.exists()
    assert manager.tracked_debug_frames_directory.exists()
    assert manager.track_crops_directory.exists()
    assert manager.florence_selected_crops_directory.exists()
    assert manager.florence_results_directory.exists()
